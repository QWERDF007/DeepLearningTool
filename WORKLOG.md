...
**目标**
- Ticket 10：验证中途写入失败或取消后无关联记录残留。

**当前状态**
- DataBase.cpp 的 copyImagesAtomic/splitDatasetAtomic 使用真实事务；测试 test_DataSplit.cpp:317 的两个取消回调始终返回 true，在事务开始前退出；移动测试只给无效目标。当前测试不足以证明中途回滚。
- 本轮未修改生产实现；应补写入发生后的失败证据，不能把入口拒绝等同于事务回滚。
- 已在原用例补充划分首项写入图片、第二项空名称失败；通过输出观察确认失败前确已插入图片，验证返回输出清空，重新打开数据库后临时数据集不存在。生产代码未改，测试尚未提交。
- 后续新增复制首项写入后取消用例：通过公开输出确认已有插入，再在取消回调返回 true；断言取消确实发生、输出清空、目标图像总数恢复。此追加修改尚未提交，未覆盖标注和标签残留。

**验证证据**
- 阅读 test_DataSplit.cpp:317 起完整测试与 DataBase.cpp:899 起事务路径；检索其余测试未找到该原子接口中途回滚的直接覆盖。
- `cmake --build build --config Release --target dltool_model_data_split_test --parallel 4` → 成功；py312 通过 run_project_tests.py 的 data-split 层复用专用负向项目 → CTest 1/1 passed，0.53 秒。
- 退化验证：临时将第二项空名称分支的 rollback 改成 commit，新断言在重开后发现临时数据集残留而失败。恢复生产代码并重建，同命令 1/1 passed，0.45 秒；git diff 确认 DataBase.cpp 无内容改动。故障注入残留仅在专用负向项目，未删除。

**下一步**
- 沿现有 ProjectDataBase 原子操作入口构造首项成功、后项失败或提交前取消，验证图像、标签与数据集回滚及输出清空；避免新建测试专用生产接口。
- 新增划分用例已完成退化敏感性验证；继续补复制和移动的实际中途失败覆盖，不把本次划分通过推广为全部原子操作通过。
