                                             ...
                                             QString *err_msg) const
{
    if (!ModelStorageService(project_dir_).ensureTestTaskStorage(model_name, task.directory_name, err_msg))
        return false;
    const QString database_path = ModelStorageService(project_dir_).testTaskDatabasePath(model_name,
                                                                                           task.directory_name);
    if (database_path.isEmpty())
                                       ...
                                       ModelTestTaskDefinition &task, QString *err_msg) const
{
    ...
    if (err_msg != nullptr && !err_msg->isEmpty())
        return false;
    const auto found = std::find_if(tasks.cbegin(), tasks.cend(), [&uuid](const auto &entry)
                                         ...
                                         QString *err_msg) const
{
    ...
    if (err_msg != nullptr && !err_msg->isEmpty())
        return false;
    auto found = std::find_if(tasks.begin(), tasks.end(), [&uuid](const auto &entry)
                              { return entry.uuid == uuid.trimmed(); });
    if (found == tasks.end())
        return setError(err_msg, QString("测试任务不存在: %1").arg(uuid));
...
bool ModelTestTaskRepository::removeTask(const QString &model_name, const QString &uuid, QString *err_msg) const
{
    ...
    {
        temporary_name = found->directory_name + QStringLiteral(".delete-")
