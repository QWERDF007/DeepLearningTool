...
{
    const EvaluationGroundTruthRecord *result = image.gt.isEmpty() ? nullptr : &image.gt.front();
...
EvaluationAggregateOutput aggregateEvaluation(const EvaluationAggregateInput &input)
{
    ...
    if (input.anomaly_detection)
    {
        output.confusion = buildAnomalyConfusionCells(anomalyConfusionSamples(input.images, input.confidence_threshold),
                                                      class_names);
    }
    else
    {
                                    ...
                                    const evaluation::CellKind kind, bool selectable, bool diagonal, bool error)
        {
            ...
            const QString row_label
                = row_fn
                    ? evaluation::matrixAxisLabel(evaluation::MatrixAxisKey::FalseNegative)
        ...
        for (auto row_it = class_names.cbegin(); row_it != class_names.cend(); ++row_it)
        {
            ...
            appendCell(row, matrix_fp, row_fp_totals.value(row_it.key()), evaluation::CellKind::FalsePositive, true,
                       false, true);
        ...
        appendCell(matrix_unmatched_fn, matrix_unmatched_fp, 0, evaluation::CellKind::NotApplicable, true, false,
                   false);
        appendCell(matrix_unmatched_fn, matrix_fp, unmatched_fn, evaluation::CellKind::FalseNegative, true, false,
                   true);
        appendCell(matrix_unmatched_fn, matrix_total, unmatched_fn, evaluation::CellKind::FalseNegativeTotal, true,
                   false, true);
        for (auto column_it = class_names.cbegin(); column_it != class_names.cend(); ++column_it)
        {
            const QString column = QString::number(column_it.key());
        ...
        appendCell(matrix_fn, matrix_unmatched_fp, unmatched_fp, evaluation::CellKind::FalsePositive, true, false,
                   true);
        ...
        appendCell(matrix_fn, matrix_total, mismatch_total + unmatched_fn, evaluation::CellKind::FalseNegativeTotal,
                   true, false, true);
        for (auto column_it = class_names.cbegin(); column_it != class_names.cend(); ++column_it)
        {
            const QString column = QString::number(column_it.key());
        ...
        appendCell(matrix_total, matrix_unmatched_fp, unmatched_fp, evaluation::CellKind::FalsePositiveTotal, true,
                   false, true);
        appendCell(matrix_total, matrix_fp, mismatch_total + unmatched_fp, evaluation::CellKind::FalsePositiveTotal,
                   true, false, true);
        appendCell(matrix_total, matrix_total, input.instances.size(), evaluation::CellKind::All, true, false, false);
