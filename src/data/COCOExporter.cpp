...
void COCOExporter::doExport(ExportDataset dataset, QString output_dir)
{
    try
    {
        ...
        for (int i = 0; i < label_count; ++i)
        {
            ...
            if ((i + 1) % std::max(1, label_count / 10) == 0 || i + 1 == label_count)
            {
                updateProgress(45 + (i + 1) * 45 / std::max(1, label_count),
                               QStringLiteral("已写入 COCO 标注 %1/%2").arg(i + 1).arg(label_count));
            }
        }
        ...
        updateProgress(100, QStringLiteral("COCO 标注文件已写入"));
        emit exportFinished(true, QStringLiteral("COCO 导出完成: %1 个图像, %2 个标注")
    ...
    catch (const std::exception &e)
    {
        ...
        emit exportFinished(false, QStringLiteral("COCO 导出失败: %1").arg(e.what()));
    }
}
