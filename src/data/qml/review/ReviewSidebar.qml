import QtQuick
import QtQuick.Templates as T
import QtQuick.Controls
import QtQuick.Layouts

import dltool.ui
import dltool.settings
import quickui

Rectangle { // 侧边栏
    id: sidebar
    color: QuiColor.Primary

    ColumnLayout {
        anchors.fill: parent
        anchors.margins: 5
        spacing: 5
        
        // 2.1 缩略图大小调整按钮
        QuiTextIconButton {
            iconSource: QuiFontIcon.ExploreContentSingle
            text: "调整缩略图大小"
            onClicked: {
                slider.from = GlobalSettings.data.labelThumbnailScaleFrom
                slider.to = GlobalSettings.data.labelThumbnailScaleTo
                slider.value = GlobalSettings.data.labelThumbnailScale
                slider.snapMode = Slider.NoSnap
                slider.stepSize = GlobalSettings.data.labelThumbnailScaleStepSize
                slider.valueUpdateCallback = function(value) {
                    GlobalSettings.data.labelThumbnailScale = value
                }
                openPopup(x, y)
            }
        }
        
        // 2.2 长宽比调整按钮
        QuiTextIconButton {
            iconSource: QuiFontIcon.Zoom
            text: "调整长宽比"
            onClicked: {
                slider.from = GlobalSettings.data.labelThumbnailAspectRatioFrom
                slider.to = GlobalSettings.data.labelThumbnailAspectRatioTo
                slider.value = GlobalSettings.data.labelThumbnailAspectRatio
                slider.snapMode = Slider.SnapAlways
                slider.stepSize = GlobalSettings.data.labelThumbnailAspectRatioStepSize
                slider.valueUpdateCallback = function(value) {
                    GlobalSettings.data.labelThumbnailAspectRatio = value
                }
                openPopup(x, y)
            }
        }
        
        // 2.3 边界调整按钮
        QuiTextIconButton {
            iconSource: QuiFontIcon.FitPage
            text: "调整边界"
            onClicked: {
                slider.from = GlobalSettings.data.labelThumbnailBorderPaddingFrom
                slider.to = GlobalSettings.data.labelThumbnailBorderPaddingTo
                slider.value = GlobalSettings.data.labelThumbnailBorderPadding
                slider.snapMode = Slider.SnapAlways
                slider.stepSize = GlobalSettings.data.labelThumbnailBorderPaddingStepSize
                slider.valueUpdateCallback = function(value) {
                    GlobalSettings.data.labelThumbnailBorderPadding = value
                }
                openPopup(x, y)
            }
        }
        
        // 2.4 亮度调整按钮
        QuiTextIconButton {
            iconSource: QuiFontIcon.Brightness
            text: "调整图像亮度"
            onClicked: {
                slider.from = GlobalSettings.ui.imageBrightnessFrom
                slider.to = GlobalSettings.ui.imageBrightnessTo
                slider.value = GlobalSettings.ui.imageBrightness
                slider.snapMode = Slider.SnapAlways
                slider.stepSize = GlobalSettings.ui.imageBrightnessStepSize
                slider.valueUpdateCallback = function(value) {
                    GlobalSettings.ui.imageBrightness = value
                }
                openPopup(x, y)
            }
        }
        
        // 2.4 对比度调整按钮
        QuiTextIconButton {
            iconSource: QuiFontIcon.BlueLight
            text: "调整图像对比度"
            onClicked: {
                slider.from = GlobalSettings.ui.imageContrastFrom
                slider.to = GlobalSettings.ui.imageContrastTo
                slider.value = GlobalSettings.ui.imageContrast
                slider.snapMode = Slider.SnapAlways
                slider.stepSize = GlobalSettings.ui.imageContrastStepSize
                slider.valueUpdateCallback = function(value) {
                    GlobalSettings.ui.imageContrast = value
                }
                openPopup(x, y)
            }
        }
        
        Item {
            Layout.fillHeight: true
        }
    }
    
    QuiPopup {
        id: popup
        bgColor: QuiColor.Primary
        closePolicy: Popup.CloseOnPressOutside
        width: 200
        height: 32
        QuiSlider {
            id: slider
            anchors.centerIn: parent
            snapMode: Slider.SnapAlways
            property var valueUpdateCallback: null
            onMoved: {
                if (valueUpdateCallback) {
                    valueUpdateCallback(value)
                }
            }
        }
        T.Overlay.modal: null // 不显示遮罩
    }

    function openPopup(x, y) {
        let pos = sidebar.mapToItem(null, x, y)
        popup.x = pos.x - popup.width - 10
        popup.y = pos.y
        popup.open()
    }
}
