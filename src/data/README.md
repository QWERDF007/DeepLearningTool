...
- `DataManager` 实现 `feature::ImageSearchDataProvider`，为图像搜索提供图像列表、路径、数据集 ID、项目路径和结果写回能力。
- 图像搜索和智能标注控制器位于 `feature` 模块，`DataManager` 继续通过 `imageSearch` 和 `smartAnnotation` 属性向 QML 暴露它们。
