...
`data` 构建目标为 `dltool_data`，默认 QML URI 为 `dltool.data`。它是项目打开后数据工作区的核心模块，负责数据集、图片、标签类别、标注实例、图片标签、过滤、统计、导入导出、图像相似搜索和智能标注控制。
...
- `ImageSearchController` 基于 InferRT + FAISS 执行以图搜图，结果写入图像搜索过滤模块。
...
- 执行图像相似度搜索，并将搜索结果纳入全局过滤。
- 执行智能标注推理，为标注页面提供辅助分割结果。
...
- 通过 InferRT/FAISS 支持图像搜索和智能标注相关能力。
