...
QString ShortcutManager::validateFormat(const QString &shortcut, const QString &owner_name) const
{
    ...
    {
        return QStringLiteral("error:%1快捷键只能是单个可打印字符").arg(owner_name);
    }
    return {};
}
...
{
    const QString format_error = validateFormat(shortcut, QStringLiteral("类别"));
    ...
    {
        return QStringLiteral("error:类别快捷键已被其他类别使用");
    }
    if (findTagId(normalized) >= 0)
    {
        return QStringLiteral("error:类别快捷键已被 Tag 使用");
    }
    return {};
}
...
QString ShortcutManager::validateTagShortcut(const QString &shortcut, const int64_t tag_id) const
{
    ...
    {
        return QStringLiteral("error:Tag 快捷键已被其他 Tag 使用");
    }
    if (findLabelClassId(normalized) >= 0)
    {
        return QStringLiteral("error:Tag 快捷键已被类别使用");
    }
    return {};
}
