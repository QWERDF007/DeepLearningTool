...
| `display_type` | 设置页控件类型，例如 `slider`、`spin`、`combo`、`checkbox`、`path`、`dir`、`folder`、`group_combo`。 |
...
| `description` | 配置项说明，用于设置页 tooltip。 |
| `enabled` | 是否允许编辑，默认 `true`。 |
| `unit` | 字段单位，可选。 |
...
5. 每个字段应配置 `description`，设置页会把它作为左侧文字区域的 tooltip。
